<div class="">
    ...
    @endif
        @if ($item)
            ...
            {{-- @if ($item->type !== 'file') --}}
            <div class="mb-4">
                <button type="button" class="btn btn-outline-primary w-100" wire:click="requestAiSuggestions"
                    wire:loading.attr="disabled" wire:target="requestAiSuggestions">
                ...
                <div class="card border-0 shadow mb-4">
                    <div class="card-body text-white ">
                        ...
                        @if ($item->type === 'texte')
                            ...
                            <button type="button" class="btn btn-light btn-sm" wire:click="applySuggestion">
                                ...
                                @foreach ($aiSuggestions['options_recommandees'] as $opt)
                                    <div class="bg-primary rounded-3 p-2 mb-2">
                                        <div class="d-flex align-items-center justify-content-between">
                                            ...
                                            <span
                                                class="badge bg-{{ $opt['priorite'] === 'haute' ? 'warning' : ($opt['priorite'] === 'moyenne' ? 'info' : 'secondary') }}">
                        ...
                        @elseif ($item->type === 'file')
                            @if (isset($aiSuggestions['document_attendu']))
                                <div class="text-primary rounded-3">
                            ...
                            @if (isset($aiSuggestions['contenu_essentiel']))
                                <div class="text-primary rounded-3">
                                    ...
                                    <ul class="small mb-0">
                                        @foreach ($aiSuggestions['contenu_essentiel'] as $contenu)
                                            <li>{{ $contenu }}</li>
                                        @endforeach
                                    </ul>
                                </div>
            ...
            <div class="card border-0 shadow-sm mb-4">
                ...
                <div class="card-body">
                    @if ($item->type === 'texte')
                        <div class="mb-3">
                            <label class="form-label fw-semibold">Votre réponse <span
                                    class="text-danger">*</span></label>
                            <textarea class="form-control @error('textValue') is-invalid @enderror" rows="8" wire:model.defer="textValue"
                    ...
                    @elseif ($item->type === 'file')
                        ...
                        @if ($submission && $submission->answers->where('kind', 'documents')->first())
                            ...
                            <div class="alert alert-info mb-3">
                                ...
                                Fichier actuel :
                                <a href="{{ Storage::disk('public')->url($existingFile->file_path) }}" target="_blank"
                                    class="alert-link">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" class="form-control @error('uploadedFile') is-invalid @enderror"
                        ...
                        @if (isset($analysisResults['score_qualite']))
                            <div class="mb-3">
                                <div class="d-flex align-items-center justify-content-between mb-2">
                                    ...
                                    <span
                                        class="badge bg-{{ $analysisResults['score_qualite'] >= 80 ? 'success' : ($analysisResults['score_qualite'] >= 50 ? 'warning' : 'danger') }}">
                        ...
                        @if (!empty($analysisResults['problemes']))
                            <div class="mb-3">
                                ...
                                @foreach ($analysisResults['problemes'] as $probleme)
                                    <div
                                        class="alert alert-{{ $probleme['severite'] === 'critique' ? 'danger' : ($probleme['severite'] === 'warning' ? 'warning' : 'info') }} py-2 mb-2">
                                        <i
                                            class="ti ti-alert-{{ $probleme['severite'] === 'critique' ? 'triangle' : 'circle' }} me-1"></i>
                                        {{ $probleme['message'] }}
                                    </div>
                                @endforeach
                            </div>
            ...
            @if (!$showConfirmation)
                <div class="d-flex gap-2 mb-3">
                    <button type="button" class="btn btn-outline-info flex-fill" wire:click="analyzeBeforeSubmit"
                        wire:loading.attr="disabled" wire:target="analyzeBeforeSubmit">
                    ...
                    <button type="button" class="btn btn-primary flex-fill" wire:click="prepareSubmit"
